...
static void BagMenu_ItemPrintCallback(u8 windowId, u32 itemIndex, u8 y)
{
    if (itemIndex != LIST_CANCEL)
    {
        ...
        else
        {
            ...
            if (itemId != ITEM_NONE)
            {
                offset = RegisteredItemIndex(itemId);
                if (offset >= 0)
                {
...
static enum Item GetSingleRegisteredItem(void) {
    u32 i;
    for (i = 0; i < ARRAY_COUNT(gSaveBlock1Ptr->registeredItems); i++)
        if (gSaveBlock1Ptr->registeredItems[i] != ITEM_NONE)
            return gSaveBlock1Ptr->registeredItems[i];
    return ITEM_NONE;
...
static void ItemMenu_Register(u8 taskId)
{
    ...
    // Only allow registration from Key Items
    if (gBagPosition.pocket != POCKET_KEY_ITEMS)
        return;
    
    index = RegisteredItemIndex(gSpecialVar_ItemId);
...
bool8 UseRegisteredKeyItemOnField(void)
{
    ...
    // Use the only registered item
        ...
        itemId = GetSingleRegisteredItem();
        if (CheckBagHasItem(itemId, 1) == TRUE) {
            gSpecialVar_ItemId = itemId;
            func = GetItemFieldFunc(itemId);
        } else {
            gSaveBlock1Ptr->registeredItems[RegisteredItemIndex(itemId)] = ITEM_NONE;
        }
    }
    if (func) {
...
// TODO: update to use IWRAM buffers and vcountatisr from new mons icons once that's merged
...
static void Task_KeyItemWheel(u8 taskId) {
    ...
    case 1: // process input
    {
        ...
        if (i == 0 || data[i] == MAX_SPRITES)
            break;
        itemId = gSaveBlock1Ptr->registeredItems[i - 1];
    ...
    case 2:
        ...
        i = CreateTask(GetItemFieldFunc(itemId), 8);
